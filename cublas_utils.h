...
template <typename T> void generate_random_matrix(int m, int n, std::vector<T>& h_arr, int *lda) {
    ...
    *lda = n;
    ...
    for (int i = 0; i < m*n; ++i) {
            h_arr.push_back(traits<T>::rand(rand_gen));
    }
}

template <typename T> bool gen_random_matrix_2_d(std::vector<T>* h_arr,int m,int n,int* lda,T** d_A) {
    generate_random_matrix(m,n,*h_arr,lda); 
    CUDA_CHECK(cudaMalloc(d_A, sizeof(T) * h_arr->size()));
    CUDA_CHECK(cudaMemcpy(static_cast<T *>(*d_A), h_arr->data(), sizeof(T) * h_arr->size(),cudaMemcpyHostToDevice));
}
